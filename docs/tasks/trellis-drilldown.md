...
### 2026-08-30: make panel boundaries explicit

The user could not tell how the old "Edit comparison / Add by" picker mapped to charts. Replace it with explicit asset inclusion, "Split panels by" (asset / any metadata key / custom), and a panel preview listing member names and counts. Allow custom per-panel membership and adding a panel. Keep grouping/count/aggregation visible above charts. Preserve scopes, overlap warnings, local edits and existing saved views.
