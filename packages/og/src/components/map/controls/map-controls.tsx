...
import { CustomDrawModes, CustomDrawModeKeys } from "../draw-modes";
import { TEXT_PRIMARY, TEXT_SECONDARY, TEXT_MUTED, TEXT_FAINT, PANEL_BG_LIGHT, INPUT_BG, HOVER_BG, ACCENT, ACCENT_15, ACCENT_30, DANGER, DANGER_BG, BORDER, BORDER_SUBTLE, BORDER_INPUT, FONT_FAMILY, BLUR_MD } from "../theme";
...
const SmallChevronIcon = ({ rotated }: { rotated: boolean }) => (
  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={TEXT_MUTED} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
...
const LayerToggle = memo(({ layers, visibleLayers, onToggle, overlay }: LayerToggleProps & { overlay?: OverlayCallbacks }) => {
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  ...
  const handleReuploadSelect = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    <div ref={ref} style={{ position: "relative" }}>
      ...
      {open && (
        ...
        >
          ...
          {layers.length > 0 && (
            <>
              ...
              {layers.map((id) => (
                <label
                  key={id}
                  style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0", cursor: "pointer", fontSize: 12, color: TEXT_PRIMARY }}
                >
                  <input
                    type="checkbox"
          ...
          {showOverlays && (
            <>
              {layers.length > 0 && <div style={{ width: "100%", height: 1, background: "rgba(148, 163, 184, 0.15)", margin: "8px 0" }} />}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 10, fontWeight: 600, color: TEXT_MUTED, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                    Overlays
                  </span>
                  {overlays.length > 0 && (
                    <span style={{ fontSize: 9, fontWeight: 600, background: ACCENT, color: "#fff", borderRadius: 8, padding: "0 5px", lineHeight: "14px" }}>
                      {overlays.length}
                    </span>
                  )}
                </div>
                {overlay.enableUpload && (
                  <button
                    ...
                    style={{
                      ...
                      border: `1px solid ${ACCENT}`, background: ACCENT_15, color: ACCENT,
                      fontSize: 10, fontWeight: 600, cursor: "pointer", fontFamily: FONT_FAMILY,
                    }}
              ...
              {overlays.map((ov) => {
                ...
                return (
                  <div key={ov.id} style={{ marginBottom: 2 }}>
                    ...
                    <div
                      style={{
                        display: "flex", alignItems: "center", gap: 6, padding: "5px 4px", borderRadius: 4,
                        background: isExpanded ? ACCENT_15 : "transparent",
                        cursor: "pointer", transition: "background 0.15s",
                      }}
                      onMouseEnter={(e) => { if (!isExpanded) e.currentTarget.style.background = HOVER_BG; }}
                      onMouseLeave={(e) => { if (!isExpanded) e.currentTarget.style.background = "transparent"; }}
                      onClick={() => setExpandedOverlayId(isExpanded ? null : ov.id)}
                    >
                      {/* Visibility checkbox */}
                      <button
                        onClick={(e) => { e.stopPropagation(); overlay.onToggle?.(ov.id); }}
                        style={{
                      ...
                      {/* Color swatch */}
                      <div style={{ width: 10, height: 10, borderRadius: 2, background: ov.style?.fillColor ?? ACCENT, border: `1px solid ${ov.style?.strokeColor ?? ACCENT}`, flexShrink: 0 }} />
                      ...
                      {isEditing ? (
                        <input
                          ...
                          onClick={(e) => e.stopPropagation()}
                          style={{
                            flex: 1, minWidth: 0, background: INPUT_BG, border: `1px solid ${ACCENT}`, borderRadius: 3,
                            color: TEXT_PRIMARY, fontSize: 11, padding: "1px 4px", outline: "none", fontFamily: FONT_FAMILY,
                          }}
                        />
                      ) : (
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 11, fontWeight: 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", color: TEXT_PRIMARY }}>
                            {ov.name}
                          </div>
                    ...
                    {isExpanded && (
                      <div style={{ padding: "6px 4px 8px 26px" }}>
                        ...
                        <div style={{ display: "flex", gap: 4, marginBottom: 8 }}>
                          <MiniAction label="Rename" onClick={() => { setEditingNameId(ov.id); setEditNameValue(ov.name); }}><SmallEditIcon /></MiniAction>
                          <MiniAction label="Re-upload" onClick={() => { setReuploadTargetId(ov.id); setTimeout(() => reuploadInputRef.current?.click(), 0); }}><UploadCloudIcon /></MiniAction>
                          <MiniAction label="Delete" danger onClick={() => overlay.onRemove?.(ov.id)}><SmallTrashIcon /></MiniAction>
                        </div>

                        {/* Style controls */}
                        <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
                          <MiniColorField label="Fill" value={ov.style?.fillColor ?? ACCENT} onChange={(v) => overlay.onUpdateStyle?.(ov.id, { fillColor: v })} />
                          <MiniColorField label="Stroke" value={ov.style?.strokeColor ?? ACCENT} onChange={(v) => overlay.onUpdateStyle?.(ov.id, { strokeColor: v })} />
                        </div>
                        <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
                          <MiniSlider label="Opacity" value={ov.style?.fillOpacity ?? 0.3} min={0} max={1} step={0.05}
                            display={`${Math.round((ov.style?.fillOpacity ?? 0.3) * 100)}%`}
                            onChange={(v) => overlay.onUpdateStyle?.(ov.id, { fillOpacity: v })} />
                          <MiniSlider label="Width" value={ov.style?.strokeWidth ?? 2} min={0.5} max={8} step={0.5}
                        ...
                        {featureCount > 0 && featureCount <= 50 && (
                          <div style={{ marginTop: 4 }}>
                            <div style={{ fontSize: 9, fontWeight: 600, color: TEXT_FAINT, textTransform: "uppercase", marginBottom: 3 }}>Features</div>
                            <div style={{ maxHeight: 140, overflowY: "auto" }}>
                              {ov.geojson.features.map((feat, idx) => {
                                ...
                                const fname = (feat.properties?.name as string) || (feat.properties?.Name as string) || `${feat.geometry?.type ?? "Feature"} ${idx + 1}`;
                                return (
                                  <div key={idx} style={{ display: "flex", alignItems: "center", gap: 4, padding: "2px 0", fontSize: 10 }}>
                                    <button
                                      onClick={() => overlay.onUpdateFeature?.(ov.id, idx, !vis)}
                                      style={{
                                        width: 13, height: 13, borderRadius: 2, flexShrink: 0,
                                        border: `1px solid ${vis ? ACCENT : TEXT_FAINT}`, background: vis ? ACCENT : "transparent",
                                        cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
                                      }}
                                    >
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => overlay.onUpdateFeature?.(ov.id, idx, undefined, { fillColor: e.target.value })}
                                      style={{ width: 12, height: 12, padding: 0, border: BORDER_INPUT, borderRadius: 2, cursor: "pointer", background: "transparent", flexShrink: 0 }}
                                    />
                                    <span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", opacity: vis ? 1 : 0.4, color: TEXT_PRIMARY }}>{fname}</span>
                                  </div>
                                );
                              })}
                            </div>
                        ...
                        {featureCount > 50 && <div style={{ fontSize: 9, color: TEXT_FAINT, marginTop: 4 }}>{featureCount} features</div>}
                      </div>
                    )}
                  </div>
                );
              ...
              {/* Hidden file inputs */}
              <input ref={fileInputRef} type="file" accept=".kmz,.kml,.geojson,.json,.zip,.shp,.dbf,.prj,.shx,.cpg,.sbn,.sbx" multiple onChange={handleFileSelect} style={{ display: "none" }} />
              <input ref={reuploadInputRef} type="file" accept=".kmz,.kml,.geojson,.json,.zip,.shp,.dbf,.prj,.shx,.cpg,.sbn,.sbx" multiple onChange={handleReuploadSelect} style={{ display: "none" }} />
            </>
          )}
        </div>
      )}
...
function MiniAction({ children, label, onClick, danger }: { children: React.ReactNode; label: string; onClick: () => void; danger?: boolean }) {
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: "4px 0", borderRadius: 4, border: BORDER_SUBTLE,
        background: "rgba(241, 245, 249, 0.6)", color: baseColor, fontSize: 9, fontWeight: 500,
        cursor: "pointer", fontFamily: FONT_FAMILY, transition: "all 0.15s",
      }}
      onMouseEnter={(e) => { e.currentTarget.style.background = danger ? DANGER_BG : HOVER_BG; e.currentTarget.style.color = danger ? DANGER : "#e2e8f0"; }}
      onMouseLeave={(e) => { e.currentTarget.style.background = "rgba(241, 245, 249, 0.6)"; e.currentTarget.style.color = baseColor; }}
    >
      {children}{label}
    </button>
  );
    ...
    <div style={{ flex: 1 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
        <input type="color" value={value} onChange={(e) => onChange(e.target.value)}
          style={{ width: 20, height: 20, padding: 0, border: BORDER_INPUT, borderRadius: 3, cursor: "pointer", background: "transparent" }} />
        <input type="text" value={value} onChange={(e) => onChange(e.target.value)}
          style={{ flex: 1, width: 0, background: INPUT_BG, border: BORDER_SUBTLE, borderRadius: 3, color: TEXT_PRIMARY, fontSize: 9, padding: "2px 4px", outline: "none", fontFamily: "monospace" }} />
      </div>
    </div>
  );
}

function MiniSlider({ label, value, min, max, step, display, onChange }: { label: string; value: number; min: number; max: number; step: number; display: string; onChange: (v: number) => void }) {
  return (
    <div style={{ flex: 1 }}>
      ...
      <input type="range" min={min} max={max} step={step} value={value}
        ...
        style={{ width: "100%", height: 3, appearance: "none", background: "rgba(148,163,184,0.2)", borderRadius: 2, outline: "none", cursor: "pointer", accentColor: ACCENT }} />
    </div>
  );
}
...
export function MapControls({
  ...
  const handleDrawSelect = useCallback((tool: DrawTool) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showCenter && (
          <ControlButton icon="focus" title="Fit to Assets" onClick={handleCenter} />
        )}

        {(showZoom || showCenter) && (showPan || showDrawing) && <div style={separatorStyle} />}

        {showPan && (
          <ControlButton icon="grab" title="Pan" active={isPanning} onClick={handlePan} />
        )}
