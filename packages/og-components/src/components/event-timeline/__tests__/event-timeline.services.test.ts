...
describe("type palette & codes", () => {
  ...
  it("gives every built-in type a short operation code", () => {
    for (const meta of Object.values(EVENT_TYPE_META)) {
      expect(meta.code).toMatch(/^[A-Z0-9]{1,4}$/);
    }
  });

  it("synthesizes a code for custom types", () => {
    expect(eventTypeMeta("casing-inspection").code).toBe("CASI");
    expect(eventTypeMeta("perf").code).toBe("PERF");
  });
});
