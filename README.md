...
- **Dynamic asset breakdowns** — Share one controlled asset/date/metadata scope across `ChartGroup`, `EventTimeline`, `ScopeFilters`, clickable metrics, contributor drill-downs, and evidence-linked summaries. Any direct `Asset.meta` key can become a dimension at runtime; chart aggregation is always explicit.
