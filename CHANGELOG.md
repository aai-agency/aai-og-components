...
- **Click a row to open a detail dialog** (accessible modal, Radix Dialog) laid out like a filled-out form: name, date, tags, description, a details/property list, and **attachments with previews** — images render inline; other files show as cards with an extension badge, name, and size. Rows show a paperclip count when an event has attachments.
- New `WellEventAttachment` type + `attachments` on `WellEvent`, and a `WellEventAttachmentSchema`. Adds `@radix-ui/react-dialog`.
