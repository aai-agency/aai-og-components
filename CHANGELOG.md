...
- **Click a row to open a detail dialog** (accessible modal, Radix Dialog) laid out like a filled-out form: an optional summary lead (a short overview for long events), then name, date, tags, description, a details/property list, and **attachments with previews** — images render inline; other files show as cards with an extension badge, name, and size. The body scrolls, so long records fit. Rows show a paperclip count when an event has attachments.
...
- New exported **`EventActivityLog`** primitive: a compact, time-based, internally-scrollable log (timestamp + event on a mini rail) for operations logs, run histories, or audit trails — drop it into `renderDetail` or use it anywhere.
